...
from aurora import dispatcher, WHITELIST_USERS, SUPPORT_USERS, SUDO_USERS, DEV_USERS, OWNER_ID, TOKEN
from aurora.__main__ import IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS, USER_SETTINGS 
from aurora.modules.disable import DisableAbleCommandHandler
from aurora.modules.log_channel import loggable, gloggable
from aurora.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin, sudo_plus, dev_plus, connection_status
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("aurora.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("aurora.modules.", 1)[1]
